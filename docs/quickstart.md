...
Pick the package from the [v0.3.1 release page](https://github.com/dlf-dds/goat-client/releases/tag/goat-client-v0.3.1). The full install + cosign-verify recipes live in the [README](../README.md#install). Short form:

```bash
# Linux (Debian / Ubuntu)
sudo dpkg -i goat-client_0.3.1_amd64.deb

# Linux (Fedora / RHEL)
sudo dnf install ./goat-client-0.3.1-1.x86_64.rpm

# macOS
hdiutil attach goat-client-0.3.1-arm64.dmg && \
  sudo installer -pkg "/Volumes/goat-client/goat-client.pkg" -target /

# Windows (PowerShell, elevated)
msiexec /i goat-client-0.3.1-amd64.msi /qn
```
